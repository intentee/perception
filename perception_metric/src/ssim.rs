...
use perception_backend::ScaleScoreWithMap;
...
mod tests {
    ...
    impl perception_backend::Backend for StubBackend {
        type Prepared = usize;
        ...
        ) -> Vec<perception_backend::ScaleDimensions> {
            (0..*prepared)
                .map(|_| perception_backend::ScaleDimensions {
                    width: 1,
                    height: 1,
                })
                .collect()
        ...
        ) -> Result<perception_backend::ScaleScore, SsimError> {
        ...
        fn compare_scale_with_map(
            ...
            _scale_index: usize,
            _adjusted_mean_exponent: f64,
        ) -> Result<perception_backend::ScaleScoreWithMap, SsimError> {
            Err(SsimError::ScaleComparison {
                width: 1,
                height: 1,
                reason: "stub backend comparison failure".to_string(),
